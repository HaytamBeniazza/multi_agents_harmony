...
from ..agents import (
    ResearchAgent, AnalysisAgent, ContentAgent, QualityAgent,
    AgentStatus, AgentMessage, AgentResult
)
...
class AgentOrchestrator:
    ...
    async def execute_research_workflow(self, topic: str, **kwargs) -> WorkflowResult:
        ...
        try:
            ...
            partial_output = self._compile_partial_output(workflow_data.get('agent_results', {}), str(e))
            ...
            return WorkflowResult(
                ...
                execution_summary={'error': str(e), 'failed_at_step': workflow_data.get('current_step', 0)},
                total_execution_time=time.time() - start_time,
                timestamp=datetime.now()
            )
    
    def _compile_final_output(self, topic: str, research_result: AgentResult, 
                              analysis_result: AgentResult, content_result: AgentResult, 
                              quality_result: AgentResult) -> Dict[str, Any]:
    ...
    def _generate_execution_summary(self, workflow_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': len([r for r in agent_results.values() if r.status == AgentStatus.COMPLETED]) / len(agent_results) if agent_results else 0,
            'quality_metrics': {
                'research_sources': len(agent_results.get('research', AgentResult('', AgentStatus.ERROR, {}, {}, 0, datetime.now())).output.get('sources', [])),
                'analysis_insights': len(agent_results.get('analysis', AgentResult('', AgentStatus.ERROR, {}, {}, 0, datetime.now())).output.get('analytical_insights', {})),
                'content_word_count': agent_results.get('content', AgentResult('', AgentStatus.ERROR, {}, {}, 0, datetime.now())).output.get('metadata', {}).get('word_count', 0),
                'final_quality_score': agent_results.get('quality', AgentResult('', AgentStatus.ERROR, {}, {}, 0, datetime.now())).output.get('overall_quality_score', 0)
            }
        }
